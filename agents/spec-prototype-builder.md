...
- **Zero Naked Metrics Craft Floor**:
  A number MUST never ship naked. Every displayed metric carries its owning noun and its unit or qualifier in the same optical unit (e.g. `128 ms`, `3 nodes`, `42 %`), labels the quantity it measures, and states its comparison basis when one is implied. A bare digit (`128`, `42`) with no unit, label, or context is a static-quality failure — the reader cannot tell what is being counted or against what. This floor is separate from `tabular-nums`: tabular alignment stabilizes the glyphs, the floor guarantees the number still means something.
...
- **Shared Shell & Multi-Surface Topology Navigation (Declared-Only Protocol)**:
  ...
  **Disabled Sibling Navigation Contract**: Sibling navigation follows actual delivery, and the quality gate asserts it. A sibling surface that IS delivered in this round MUST be reachable by a live `href` from every other delivered member. A sibling surface that is NOT delivered MUST NOT be linked by a live `href` — the target would 404 and the coverage assertion fails. It must stay review-visible instead as a rendered, non-navigating affordance: a disabled control (`<button disabled aria-disabled="true">`) or an explicit textual/data representation (`<span data-sibling="<slice-id>">…(未交付)</span>`), never `display: none` and never a dangling link. The undelivered destination remains visible to review; only its reachability is withheld.
