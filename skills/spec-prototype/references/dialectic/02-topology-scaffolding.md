...
### Option A: Synchronized Multi-Column Workbench (对标 Linear / CAD)
```text
┌─ Source / Object List ─┐┌─ Central Resolution Matrix ─┐┌─ Authority Gate / Inspector ─┐
│ • Pinned entities (320px)││ • Side-by-side diff & form ││ • Invariants readiness checklist│
│ • State status chips    ││ • Decisive inputs (flex)    ││ • Sealed commit trigger (280px)│
└─────────────────────────┘└──────────────────────────────┘└────────────────────────────────┘
```
- *Trade-off*: Maximum information throughput, zero modal disruption; requires >=1280px viewport.

### Option B: Focused Progressive Flow with Drawer (对标 Stripe / Notion)
```text
┌─ Global Entity Header & Primary Metrics ──────────────────────────────────────────────────┐
│ Primary workspace table / list (840px measure max)                                       │
│ [Row Action] ──► Discloses contextual slide-over drawer for parameter adjustment          │
└───────────────────────────────────────────────────────────────────────────────────────────┘
```
...
## 3. Visual-First Inline Layout Prototypes (以图促思 · 告别纯文字脑补)
When presenting options to human stakeholders, accompany structural ASCII with lightweight, inline CSS Grid / SVG preview blocks whenever rich display surfaces are available:
```html
<!-- Mini Workspace Grid Specimen -->
<div style="display:grid; grid-template-columns: 240px 1fr 300px; height: 180px; gap: 8px; font-family: sans-serif; font-size: 11px;">
  <div style="background: #161b22; border: 1px solid #30363d; border-radius: 6px; padding: 8px;">Navigation & Filters</div>
  <div style="background: #0d1117; border: 1px solid #30363d; border-radius: 6px; padding: 8px;">Primary Workspace Canvas</div>
  <div style="background: #161b22; border: 1px solid #30363d; border-radius: 6px; padding: 8px;">Inspector & Actions</div>
</div>
```
Stakeholders can review spatial hierarchy visually within seconds.

## 4. Interaction Resistance & OOUX Card Ladder
