...
- **Concentric Border Radius Floor**: Nested container corners must maintain concentricity: $R_{inner} = \max(0, R_{outer} - padding)$. Mismatched, pinched, or bulging nested corner radii distort visual harmony and fail the Floor.
- **Tabular Numerics Floor**: All dynamic metrics, data tables, financial figures, telemetry counters, and timers must declare `font-variant-numeric: tabular-nums` (or CSS token `--font-tabular`) to prevent jarring layout twitching during value mutations.
- **Optical Alignment Floor**: Asymmetric shapes and directional icons (e.g. play triangles, directional arrows, disclosure chevrons) must be optically nudged (1-2px) to align with human visual center of mass rather than relying on cold geometric bounding-box centering.
- **Atmospheric Undertone Floor (Anti-Sterile Gray)**: Canvases and card surfaces must avoid dead, sterile neutral gray (`#808080` or ungrounded `gray-500` washes). Palette must reflect deliberate chromatic undertones (e.g. cobalt-tinted dark space, warm graphite, or parchment tint) that establish tangible atmosphere without sacrificing WCAG contrast.
- **Vague-Word Firewall Floor**: Architectural records, specifications, or design rationale that rely on ungrounded aesthetic buzzwords ("高级感", "精致", "大气", "克制", "premium", "elegant", "sophisticated") fail the Floor. Every aesthetic choice must be justified with concrete parameters (token values, contrast ratios, spacing multipliers, and transition curves).
- **Physical Metaphor & Kinetic Accountability (Non-Transfer Boundary)**: Where a component claims a physical, optical, biomorphic or domain tool mapping, the implementation must genuinely express the stated phenomenon through observable interactive or visual behavior. The design must explicitly declare its *non-transfer boundary* (e.g. tactile detent transfers, but decorative skeuomorphic noise or faux-metal chrome does not). Hollow cosmetic styling that merely names a metaphor without enacting it fails the Floor. Interactive buttons and levers must implement tactile feedback (`:active { transform: scale(0.97); }`) and natural deceleration curves (`cubic-bezier(0.16, 1, 0.3, 1)`).
- **Data Floor (Zero Naked Metrics)**: In operational, analytical, or telemetry surfaces, metrics and sparklines must never appear unanchored. Every key metric must provide a visible reference benchmark (threshold line, normal baseline, unit scale, or event milestone) so users can immediately evaluate whether a value indicates health or danger.
- **Content Mechanics Floor (Action Verb Lifecycle Closure)**: Action verbs must maintain exact semantic continuity across the user lifecycle: trigger button (e.g. "Quarantine"), dialog heading ("Quarantine Worker"), primary commit action ("Quarantine"), and post-completion toast must share identical vocabulary. Mid-journey semantic shifts (e.g. "Quarantine" -> "Isolate" -> "Archived") fail the Floor.
- **Decisive Exchange 3-Frame Floor**: Primary operational levers must be inspectable across three explicit states: `Intent` (hover/focus cue), `Detent` (tactile resistance, `:active scale(0.97)`), and `Settled` (post-action confirmation with reversible escape).
